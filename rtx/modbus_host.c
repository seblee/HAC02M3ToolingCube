...
static void MODH_AnalyzeApp(void)
{
    ...
    osSignalSet(tid_testThread, g_tModH.RxBuf[1]);
}
 ...
 */
void MODH_RxData(MODH_T *tmodh, uint8_t *src, uint8_t len)
{
    if ((tmodh->RxCount + len) > H_RX_BUF_SIZE)
        tmodh->RxCount = 0;
    memcpy(tmodh->RxBuf + tmodh->RxCount, src, len);
    tmodh->RxCount += len;
    g_modh_timeout = 0;
    MODH_Poll();
}
...
void MODH_Poll(void)
{
    ...
    */
    ...
    {
        if ((g_tModH.RxBuf[i] == SlaveHMIAddr) || (g_tModH.RxBuf[i] == SlaveBoardAddr))
        {
            memmove(&g_tModH.RxBuf[0], &g_tModH.RxBuf[i], g_tModH.RxCount - i);
            g_tModH.RxCount -= i;
            break;
        }
    }
        ...
        switch (g_tModH.Reg01H)
        {
            case REG_D01:
                if (bytes >= 1)
                {
                    ...
                    g_tVar[g_tModH.RxBuf[0] - 1].D01 = BEBufToUint16(p);
                    p += 2; /* 寄存器 */
                    g_tVar[g_tModH.RxBuf[0] - 1].D02 = BEBufToUint16(p);
                    p += 2; /* 寄存器 */
                    g_tVar[g_tModH.RxBuf[0] - 1].D03 = BEBufToUint16(p);
                    p += 2; /* 寄存器 */
                    g_tVar[g_tModH.RxBuf[0] - 1].D04 = BEBufToUint16(p);
                    p += 2; /* 寄存器 */
        ...
        switch (g_tModH.Reg02H)
        {
            case REG_T01:
                if (bytes == 6)
                {
                    ...
                    g_tVar[g_tModH.RxBuf[0] - 1].T01 = BEBufToUint16(p);
                    p += 2; /* 寄存器 */
                    g_tVar[g_tModH.RxBuf[0] - 1].T02 = BEBufToUint16(p);
                    p += 2; /* 寄存器 */
                    g_tVar[g_tModH.RxBuf[0] - 1].T03 = BEBufToUint16(p);
                    p += 2; /* 寄存器 */
        ...
        switch (g_tModH.Reg04H)
        {
            case REG_T01:
                if (bytes == 2)
                {
                    ...
                    g_tVar[g_tModH.RxBuf[0] - 1].A01 = BEBufToUint16(p);
                    p += 2; /* 寄存器 */
...
static void MODH_Read_05H(void)
{
    if (g_tModH.RxCount > 0)
    {
        if (g_tModH.RxBuf[0] == SlaveHMIAddr)
        {
...
static void MODH_Read_06H(void)
{
    if (g_tModH.RxCount > 0)
    {
        if ((g_tModH.RxBuf[0] == SlaveHMIAddr) || (g_tModH.RxBuf[0] == SlaveBoardAddr))
        {
...
void MODH_Read_03H(void)
{
    ...
    if (g_tModH.RxCount > 0)
    {
        ...
        for (i = 0; i < (bytes / 2); i++)
        {
            g_tVar[g_tModH.RxBuf[0] - 1].P01[i] = BEBufToUint16(p);
            p += 2; /* 寄存器 */
        }
        g_tModH.fAck03H = 1;
    }
