 ...
 CLEAR_BIT(CAN1->MCR, CAN_MCR_SLEEP);
  ...
  /* USER CODE BEGIN CAN_Init 2 */
    {        
        ...
        sFilterConfig.FilterActivation     = CAN_FILTER_ENABLE;  //激活活过滤器
        ...
        // /*************************************************************************************************************************
        // 屏蔽寄存器的设置这里的???路是先将标准CAN ID和扩展CAN ID对应的ID值先异或后取反，为什么？异或是为了找出两个CAN
        // ID有哪些位是相同的，是相同的位则说明需
        // 要关心，??要关心的位对应的屏蔽码位应该设置??1，因此需要取反一下???最后再整体左移3位???
        // ****************************************************************************************************************************/

        // mask = (Std_ID << 18); /*这里为什么左??18位？因为在标准CAN
        //          ID占ID18~ID28，为了与CAN_FilterIdHigh对齐，应左移2位，接着为了与扩??
        //          CAN对应，还应该再左??16位，因此，???共应左??2 + 16??18位???
